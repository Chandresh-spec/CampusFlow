...
export default function TeacherDashboard() {
  const { isAuthorized, isLoading: authLoading } = useRoleGuard(['faculty', 'admin', 'student']);
  ...
  const { data: dashboardData } = useQuery({
    queryKey: ['teacherStudentDashboard', selectedSem, search],
    queryFn: async () => {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-800 flex flex-col md:flex-row relative font-sans antialiased">
      ...
      <aside className="w-full md:w-64 lg:w-72 bg-white border-r border-slate-200/80 shrink-0 flex flex-col justify-between p-6 z-20">
        <div>
          {/* Logo */}
          <Link href="/teacher" className="flex items-center gap-3 mb-8 group">
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-y-auto">
        {/* Top Header Bar */}
        <header className="px-6 md:px-10 py-4 flex items-center justify-between border-b border-slate-200/60 bg-white/60 backdrop-blur-md sticky top-0 z-30">
          <div className="flex items-center gap-3">
            <Link
              href="/my-uploads"
              className="inline-flex items-center gap-2 bg-[#059669] hover:bg-[#047857] text-white text-xs font-semibold px-4 py-2 rounded-xl transition shadow-sm"
            >
              <Plus size={15} /> Upload Note / Material
            </Link>
          </div>
